...
class Down_att(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, norm_layer=None):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleCv(in_channels * 2, out_channels, norm_layer=norm_layer)
        )
...
class Up_att(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True, norm_layer=None):
        ...
        if bilinear:
            ...
            self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2)
            self.conv = DoubleCv(in_channels*2, out_channels, norm_layer=norm_layer)

        # self.conv2 = DoubleCv(in_channels * 2, in_channels, norm_layer=norm_layer)

    def forward(self, x1, x2, y, att):
        if y != None:
            x1 = x1 + y
        x1 = self.up(x1)
        # input is CHW
...
class AddOutCv_kernel2(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, n_residual_blocks=9):
        super(AddOutCv_kernel2, self).__init__()
